                ...
                <div ng-show="cell.status == 'invalid' || cell.span > 0" class="col-xs-{{3*cell.span || 3}}">
                  <div ng-hide="cell.status == 'invalid'">
                    ...
                    <div class="form-group">
                      ...
                      <!-- Normal text input -->
                      <input ng-if="cell.type == 'Text'" class="form-control" type="text" ng-model="$ctrl.volunteer[cell.name]" placeholder="{{cell.label}}">

                      <!-- Textarea input -->
                      <textarea ng-if="cell.type == 'Textarea'" class="form-control" ng-model="$ctrl.volunteer[cell.name]" placeholder="{{cell.label}}"></textarea>

                      <!-- Date input -->
                      <input ng-if="cell.type == 'Date'" class="form-control" type="date" ng-model="$ctrl.volunteer[cell.name]">

                      <!-- Radio buttons -->
                      <div ng-if="cell.type == 'Radio Buttons'">
                        <div ng-repeat="choice in cell.choices.split(',')" {{cell.choices}} <label class="radio-inline">
                          <input name="{{cell.name}}" type="radio" ng-model="$ctrl.volunteer[cell.name]" value="{{choice.trim()}}"> {{choice.trim()}}
                          </label>
                        </div> <!-- Repeat choices -->
                      </div> <!-- Radio buttons -->
